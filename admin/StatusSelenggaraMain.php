...
$maxRows_rcd_Status = 20;
$pageNum_rcd_Status = 0;
if (isset($_GET['pageNum_rcd_Status'])) {
  $pageNum_rcd_Status = $_GET['pageNum_rcd_Status'];
}
$startRow_rcd_Status = $pageNum_rcd_Status * $maxRows_rcd_Status;

mysql_select_db($database_dataconn, $dataconn);
$query_rcd_Status = "SELECT * FROM statusselenggara ORDER BY Keterangan ASC";
$query_limit_rcd_Status = sprintf("%s LIMIT %d, %d", $query_rcd_Status, $startRow_rcd_Status, $maxRows_rcd_Status);
$rcd_Status = mysql_query($query_limit_rcd_Status, $dataconn) or die(mysql_error());
$row_rcd_Status = mysql_fetch_assoc($rcd_Status);

if (isset($_GET['totalRows_rcd_Status'])) {
  $totalRows_rcd_Status = $_GET['totalRows_rcd_Status'];
} else {
  $all_rcd_Status = mysql_query($query_rcd_Status);
  $totalRows_rcd_Status = mysql_num_rows($all_rcd_Status);
}
$totalPages_rcd_Status = ceil($totalRows_rcd_Status/$maxRows_rcd_Status)-1;

$queryString_rcd_Status = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rcd_Status") == false && 
        stristr($param, "totalRows_rcd_Status") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rcd_Status = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rcd_Status = sprintf("&totalRows_rcd_Status=%d%s", $totalRows_rcd_Status, $queryString_rcd_Status);
?>
      ...
      <tr>
        ...
        <form id="form1" name="form1" method="post" action="DataMain.php">
         <table width="90%" border="0" align="center">
           <tr>
                <td align="center"><h3>SENARAI STATUS SELENGGARA</h3></td>
              </tr>
              <tr>
                <td align="center">&nbsp;</tr>
               <tr>
              ...
              <tr>
                <td align="right"><a href="StatusSelenggaraAdd.php"><img src="../images/add.gif" width="16" height="16" /> Tambah Status Selenggara</a></td>
              </tr>
          </table>
           ...
           <table width="90%" border="1" align="center"  id="newspaper-b">
            ...
            <?php  $counter = ($startRow_rcd_Status + 1);?>
            <?php do { ?>
              <tr>
    <td align="center"><?php echo $counter ?></td>
    <td><?php echo $row_rcd_Status['Keterangan']; ?></td>
    <td align="center"><a href="StatusSelenggaraEdit.php?Id=<?php echo $row_rcd_Status['Id']; ?>">
      <img src="../images/edit.png"  width="16" height="16" /></a></td>
  </tr> <?php $counter++; ?>
              <?php } while ($row_rcd_Status = mysql_fetch_assoc($rcd_Status)); ?>
           </table>
 <p align="center" ><span class="px10">
 Rekod <?php echo ($startRow_rcd_Status + 1) ?>
 Hingga <?php echo min($startRow_rcd_Status + $maxRows_rcd_Status, $totalRows_rcd_Status) ?>
 Daripada </span><?php echo $totalRows_rcd_Status ?></p>

<table width="20%" border="0" cellpadding="1" align="center">
    <tr>
    <td align="center" width="25%"><a href="<?php printf("%s?pageNum_rcd_Status=%d%s", $currentPage, 0, $queryString_rcd_Status); ?>"><img src=	"/bas2/images/paging_far_left.png" name="first" width="24" height="24" border="0" id="first" /></a></td>
    <td align="center" width="25%"><a href="<?php printf("%s?pageNum_rcd_Status=%d%s", $currentPage, max(0, $pageNum_rcd_Status - 1), $queryString_rcd_Status); ?>"><img src="/bas2/images/paging_left.png" name="previous" width="24" height="24" border="0" id="previous" /></a></td>
    <td align="center" width="25%"><a href="<?php printf("%s?pageNum_rcd_Status=%d%s", $currentPage, min($totalPages_rcd_Status, $pageNum_rcd_Status + 1), $queryString_rcd_Status); ?>"><img src="/bas2/images/paging_right.png" name="next" width="24" height="24" border="0" id="next" /></a></td>
    <td align="center" width="25%"><a href="<?php printf("%s?pageNum_rcd_Status=%d%s", $currentPage, $totalPages_rcd_Status, $queryString_rcd_Status); ?>"><img src="/bas2/images/paging_far_right.png" alt="" name="last" width="24" height="24" border="0" id="last" /></a></td>
    </tr>
</table>
          </form>
        <br/><br/>
