...
$maxRows_rcd_Staf = 20;
$pageNum_rcd_Staf = 0;
if (isset($_GET['pageNum_rcd_Staf'])) {
  $pageNum_rcd_Staf = $_GET['pageNum_rcd_Staf'];
}
$startRow_rcd_Staf = $pageNum_rcd_Staf * $maxRows_rcd_Staf;

mysql_select_db($database_dataconn, $dataconn);
if (isset($_POST['Search'])) {
$query_rcd_Staf = sprintf("SELECT Id, Nama, NoMykad, Bahagian FROM staf 
WHERE Nama like %s
ORDER BY Nama ASC",
GetSQLValueString("%" .$Search. "%", "text"));
}else{
$query_rcd_Staf = "SELECT Id, Nama, NoMykad, Bahagian FROM staf ORDER BY Nama ASC";
}
$query_limit_rcd_Staf = sprintf("%s LIMIT %d, %d", $query_rcd_Staf, $startRow_rcd_Staf, $maxRows_rcd_Staf);
$rcd_Staf = mysql_query($query_limit_rcd_Staf, $dataconn) or die(mysql_error());
$row_rcd_Staf = mysql_fetch_assoc($rcd_Staf);

if (isset($_GET['totalRows_rcd_Staf'])) {
  $totalRows_rcd_Staf = $_GET['totalRows_rcd_Staf'];
} else {
  $all_rcd_Staf = mysql_query($query_rcd_Staf);
  $totalRows_rcd_Staf = mysql_num_rows($all_rcd_Staf);
}
$totalPages_rcd_Staf = ceil($totalRows_rcd_Staf/$maxRows_rcd_Staf)-1;

$queryString_rcd_Staf = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rcd_Staf") == false && 
        stristr($param, "totalRows_rcd_Staf") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rcd_Staf = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rcd_Staf = sprintf("&totalRows_rcd_Staf=%d%s", $totalRows_rcd_Staf, $queryString_rcd_Staf);
?>
    ...
    <table width="100%" border="0" cellspacing="10" cellpadding="0">
      <tr>
		...
		<!-- InstanceBeginEditable name="Content" -->
         <form id="form1" name="form1" method="post" action="StaffMain.php">
         <table width="90%" border="0" align="center">
           <tr>
                <td align="center"><h3>SENARAI STAF</h3></td>
              </tr>
              <tr>
                <td align="center">&nbsp;</tr>
               <tr>
              ...
              <tr>
                <td align="right"><a href="StaffAdd.php"><img src="../images/add.gif" width="16" height="16" /> Tambah Staf</a></td>
              </tr>
          </table>
              ...
              <th scope="col" align="center" width="50px">Bil</th>
                ...
                <th scope="col" align="center">No MyKad</th>
                <th scope="col" align="center">Bahagian/Unit</th>
                <th scope="col" width="50px">&nbsp;</th>
              </tr>
     		</thead>
              <?php  $counter = ($startRow_rcd_Staf + 1); do { ?>
                <tr>
                  <td align="center"><?php echo $counter ?></td>
                  <td><?php echo $row_rcd_Staf['Nama']; ?></td>
                  <td><?php echo $row_rcd_Staf['NoMykad']; ?></td>
                  <td><?php echo $row_rcd_Staf['Bahagian']; ?></td>
                  <td align="center">
                  <a href="StaffEdit.php?Id=<?php echo $row_rcd_Staf['Id']; ?>">
                  <img src="../images/edit.png" width="16" height="16" /></a> |  
                  <a href="StaffInventory.php?Id=<?php echo $row_rcd_Staf['Id']; ?>">
                  <img src="../images/report.png" width="16" height="16" /></a>
                  </td>
                </tr>
                <?php $counter++; } while ($row_rcd_Staf = mysql_fetch_assoc($rcd_Staf)); ?>
            </table>
      <p align="center" ><span class="px10">Rekod 
	  <?php echo ($startRow_rcd_Staf + 1) ?> Hingga
	  <?php echo min($startRow_rcd_Staf + $maxRows_rcd_Staf, $totalRows_rcd_Staf) ?> Daripada 
	  <?php echo $totalRows_rcd_Staf ?></span>
      </p>
